...
export default function Home(){
 ...
 const formIndex=agent.seed%20; const armsIndex=(agent.seed>>>8)%20; const backgroundIndex=(agent.seed>>>16)%20;
 function atlasPosition(index:number){const column=index%5;const row=Math.floor(index/5);return `${column*25}% ${row*(100/3)}%`}
 ...
 return <main className="shell" onClickCapture={acknowledgeClick}>
  ...
  {activeDid&&<><section className="workspace cyber-workspace">
   ...
   <section className="main-panel panel cyber-stage"><div className="panel-label">8,000 LAYER COMBINATIONS <span>DID-DETERMINISTIC</span></div><div className="hero-character"><div className="layered-sprite" role="img" aria-label={`Cybernetic body ${formIndex+1}, arms ${armsIndex+1}, and environment ${backgroundIndex+1}, selected deterministically from the entered DID`}><i className="sprite-layer sprite-background" style={{backgroundPosition:atlasPosition(backgroundIndex)}}/><i className="sprite-layer sprite-body" style={{backgroundPosition:atlasPosition(formIndex)}}/><i className="sprite-layer sprite-arms" style={{backgroundPosition:atlasPosition(armsIndex)}}/><span>BODY {String(formIndex+1).padStart(2,'0')} · ARMS {String(armsIndex+1).padStart(2,'0')} · SCENE {String(backgroundIndex+1).padStart(2,'0')}</span></div></div>
