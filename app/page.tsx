...
export default function Home(){
 ...
 const formIndex=agent.seed%20; const backgroundIndex=(agent.seed>>>16)%20;
 ...
 return <main className="shell" onClickCapture={acknowledgeClick}>
  ...
  {activeDid&&<><section className="workspace cyber-workspace">
   ...
   <section className="main-panel panel cyber-stage"><div className="panel-label">400 VISUAL COMBINATIONS <span>DID-DETERMINISTIC</span></div><div className="hero-character"><div className="layered-sprite" role="img" aria-label={`Cybernetic form ${formIndex+1} and environment ${backgroundIndex+1}, selected deterministically from the entered DID`}><i className="sprite-layer sprite-background" style={{backgroundPosition:atlasPosition(backgroundIndex)}}/><i className="sprite-layer sprite-form" style={{backgroundPosition:atlasPosition(formIndex)}}/><span>FORM {String(formIndex+1).padStart(2,'0')} · SCENE {String(backgroundIndex+1).padStart(2,'0')}</span></div></div>
